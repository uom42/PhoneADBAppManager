...
namespace AndroidADBAppManager
{
	partial class frmMain
	{
		...
		{
			txtPropsList_Filter.AttachDelayedFilter(Property_FillWithFilter);
			{
		...
		private void Property_FillWithFilter()
		{
			...
			lvwPropsList.ExecOnLockedUpdate(() =>
			{
				lvwPropsList.ClearItems();
				...
				try
				{
					...
					foreach (var P in aFiltered)
					{
						...
						if (sKey.Contains('.'))
						{
							...
							var crg = lvwPropsList.GroupsCreateGroupByKey(groupHeader, null, ListViewGroupCollapsedState.Expanded);
							lvg = crg.Group;
						}
						P.Group = lvg;
					}
				...
				finally { lvwPropsList.SetGroupsTitlesFast(); }
			}, true);
		}
	}
}
