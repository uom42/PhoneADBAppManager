...
enabled\=(?<enabled>\d)";
		...
		internal Object? KnownPackageData { get; private set; } = null;
		...
		internal Package(ADBServer parent, string adbPackageString) : base()
		{
			if (adbPackageString.e_IsNullOrWhiteSpace()) throw new ArgumentNullException(nameof(adbPackageString));

			const string GOOD_PACKAGE_PREFIX = "package:";
			const string C_OLD_SEPARATOR = "=";

			srv = parent;
			adbPackageString = adbPackageString.Trim();
			// package:/system/priv-app/AutomationTest_FB/AutomationTest_FB.apk=com.sec.android.app.DataCreate'

			if (!adbPackageString.ToLower().StartsWith(GOOD_PACKAGE_PREFIX))
				throw new Exception(adbPackageString + " NOT FOUND PREFIX " + GOOD_PACKAGE_PREFIX);

			adbPackageString = adbPackageString.Substring(GOOD_PACKAGE_PREFIX.Length);
			...
			int iSeparatorOld = adbPackageString.LastIndexOf(C_OLD_SEPARATOR);
			if (iSeparatorOld < 1)
				throw new Exception($"{adbPackageString} NOT FOUND Path/PackageName SEPARATOR ('{C_OLD_SEPARATOR}')!");

			Path = adbPackageString.Substring(0, iSeparatorOld).Trim();
			Name = adbPackageString.Substring(iSeparatorOld + 1).Trim();
		...
		internal async Task GetDumpsysInfo()
		{
			...
			string dumpSys = (await srv!.e_runADBAsync(sCMD, 30)).NormalizedOutput();
		...
		internal async Task Enable(ADBServer Server, bool bEnable)
		{
			...
			sCMD += " " + Name;
			string sActionResult = (await Server.e_runADBAsync(sCMD, 30)).Output;
		...
		internal async Task Install(ADBServer Server, bool bInstall)
		{
			...
			else
			{
				...
				string sActionResult = (await Server.e_runADBAsync(sCMD,
					30 * 3
					)).Output;
		...
		internal async Task<System.IO.FileInfo> Download(ADBServer Server, string localPCFolderToSave)
		{
			// adb pull -a /system/priv-app/SOAgent/SOAgent.apk t:\1.apk
			// 1928 KB/s (39502 bytes in 0.020s)


			var fiFile = Path.e_ToFileInfo();
			string sPCAPKFile = Name + "." + fiFile!.Name;
			fiFile = System.IO.Path.Combine(localPCFolderToSave, sPCAPKFile).e_ToFileInfo();
			string sCMD = $"pull -a {Path} \"{fiFile!.FullName}\"";
			string sDownloadResult = (await Server.e_runADBAsync(sCMD, 60)).NormalizedOutput(); // 1928 KB/s (39502 bytes in 0.020s)

			if (fiFile.Exists) return fiFile;
		...
		public static bool operator ==(Package P1, Package P2) => (P1.ToString().ToLower() ?? "") == (P2.ToString().ToLower() ?? "");
