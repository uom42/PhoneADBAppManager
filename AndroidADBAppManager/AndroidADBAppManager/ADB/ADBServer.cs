...
namespace AndroidADBAppManager.ADB
{
	internal class ADBServer
	{
		...
		public ADBServer([NotNull] string pathADBexe)
		{
			...
			var closeResult = e_runADB("kill-server");
			var startResult = e_runADB("start-server");
			...
			var aDevices = GetAttachedDevices().e_RunSync();
		...
		internal async Task<ConsoleOutput> e_runADBAsync(
			string adbCommand,
			int timeout_sec = DEFAULT_ADB_TIMEOUT_SEC)
				=> await e_runADBAsync_Core(adbPath, adbCommand, timeout_sec);


		internal static async Task<ConsoleOutput> e_runADBAsync_Core(
			string adbPath,
			string adbCommand,
			int timeout_sec = DEFAULT_ADB_TIMEOUT_SEC)
		{
			using (Task<ConsoleOutput> tskExecADB = new(
				() => e_runADB(adbPath, adbCommand, timeout_sec),
		...
		internal static ConsoleOutput e_runADB(
			string adbPath,
			string adbCommand,
			int timeout_sec = DEFAULT_ADB_TIMEOUT_SEC)
		...
		internal ConsoleOutput e_runADB(
			string adbCommand,
			int timeout_sec = DEFAULT_ADB_TIMEOUT_SEC)
				=> e_runADB(adbPath, adbCommand, timeout_sec);
	...
	'    '#adb Get-serialno
	'    Dim sDevicesIDs = Await e_runADB("get-serialno")
	...
	'End Function
		...
		public async Task<Device[]> GetAttachedDevices()
		{
			...
			string sDevicesList = (await e_runADBAsync("devices -l")).Output.NormalizeConsoleOutput();
			...
			if (sDevicesList.e_IsNullOrWhiteSpace()) return Array.Empty<Device>();

			var asDevicesList = sDevicesList.e_SplitToLines(true);
		...
		{
			var sProps = (await e_runADBAsync("shell getprop")).NormalizedOutput();
		...
		private async Task<Package[]> GetPackages_Core(GET_PACKAGES_FLAGS Flags = GET_PACKAGES_FLAGS.All)
		{
			...
			string sPackagesListString = (await e_runADBAsync(sAdbCmd, 20)).NormalizedOutput();
			...
			string[] packageStrings = sPackagesListString
				.e_SplitToLines(true)
				.ToArray();// .Split(ExtensionsString.LF.e_ToArrayOf(), StringSplitOptions.RemoveEmptyEntries).ToArray();

			if (!packageStrings.Any()) throw new Exception("Not Found Any Packages!");

			string[] AAA = packageStrings
				.Where(s => s.e_IsNullOrWhiteSpace())
				.ToArray();

			Package[] aPackages = packageStrings
				.Select(s => new Package(this, s)).ToArray();
