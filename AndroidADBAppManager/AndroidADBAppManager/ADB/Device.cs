...
namespace AndroidADBAppManager.ADB
{
	internal class Device
	{
		...
		internal Device(string deviceString) : base()
		{
			...
			if (deviceString.e_IsNullOrWhiteSpace())
		...
		public static string AllConnectedDevicesToString(Device[] devices)
		{
			var AA = devices
				.Select(rDev => rDev.ToString())
				.ToArray();

			string A = string.Join(ExtensionsConsole.CRLF, AA);
			return A;
		}
	}
}
